  ...
  $.post("git", cmd, function (data, status, xhr) {
    if (xhr.status == 200) {
      ...
      .fail(function (xhr, status, error) {
        webui.showError(i18n.SERVICE_NOT_RUNNING);
      });
};
...
`)[0];
  ...
  self.fetchSection($("#sidebar-local-branches", self.element)[0], i18n.LOCAL_BRANCHES, "local-branches", "branch");
  self.fetchSection($("#sidebar-remote-branches", self.element)[0], i18n.REMOTE_BRANCHES, "remote-branches", "branch --remotes");
  self.fetchSection($("#sidebar-tags", self.element)[0], i18n.TAGS, "tags", "tag");
};
  ...
  self.populate = function () {
    ...
    webui.git("log --date-order --pretty=raw --decorate=full --max-count=" + (maxCount + 1) + " " + self.nextRef + " --", function (data) {
      ...
      if (self.nextRef != undefined) {
        ...
        $(`<a class="list-group-item-text">${i18n.SHOW_PREVIOUS_COMMITS}</a>`).appendTo(moreTag[0]);
    ...
    html +=
        `
          <div class="panel-heading btn-toolbar" role="toolbar">
              <button type="button" class="btn btn-sm btn-default diff-ignore-whitespace" data-toggle="button">${i18n.IGNORE_WHITESPACES}</button>
              <button type="button" class="btn btn-sm btn-default diff-context-all" data-toggle="button">${i18n.COMPLETE_FILE}</button>
              ...
              <div class="btn-group btn-group-sm diff-selection-buttons">
                  <button type="button" class="btn btn-default diff-stage" style="display:none">${i18n.STAGE}</button>
                  <button type="button" class="btn btn-default diff-cancel" style="display:none">${i18n.CANCEL}</button>
                  <button type="button" class="btn btn-default diff-unstage" style="display:none">${i18n.UNSTAGE}</button>
              </div>
              ${sideBySide ? '' : `<button type="button" class="btn btn-sm btn-default diff-explore">${i18n.EXPLORE}</button>`}
          </div>
`;
  }
  ...
  $(".diff-stage", self.element).click(function () {
    self.applySelection(false, true);
  });
  $(".diff-cancel", self.element).click(function () {
    self.applySelection(true, false);
  });
  $(".diff-unstage", self.element).click(function () {
    self.applySelection(true, true);
  });

  $(".diff-explore", self.element).click(function () {
...
webui.FileListView = function (commitExplorerView, files) {
  ...
  self.element = $(`
<div class="file-list-view panel panel-default">
...
webui.CommitView = function (historyView) {
  ...
  var buttonBox = new webui.TabBox([[i18n.COMMIT, self.showDiff], [i18n.TREE, self.showTree]]);
...
webui.WorkspaceView = function (mainView) {
  ...
  self.workingCopyView = new webui.ChangedFilesView(self, "working-copy", i18n.WORKING_COPY);
  ...
  self.stagingAreaView = new webui.ChangedFilesView(self, "staging-area", i18n.STAGIN_AREA);
...
webui.CommitMessageView = function (workspaceView) {
  ...
  self.onCommit = function () {
    if (workspaceView.stagingAreaView.filesCount == 0 && !amend.hasClass("active")) {
      webui.showError(i18n.NO_FILES_STAGED_FOR_COMMIT);
    } else if (textArea.value.length == 0) {
      webui.showError(i18n.ENTER_MESSAGE);
    } else {
      var cmd = "commit ";
  ...
  self.element = $(`
        <div id="commit-message-view" class="panel panel-default">
            <div class="panel-heading">
                <h5>${i18n.MESSAGE}</h5>
                <div class="btn-group btn-group-sm">
                    <button type="button" class="btn btn-default commit-message-amend" data-toggle="button">${i18n.AMEND}</button>
                    <button type="button" class="btn btn-default commit-message-commit">${i18n.COMMIT}</button>
                </div>
            </div>
            <textarea></textarea>
        </div>
